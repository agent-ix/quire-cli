...
* **2026-09-22** — **The engine SHA is stated in `Cargo.toml` only (PLAT-974).** [FR-020](./functional/FR-020-assurance-export-subcommand.md)-CON-1 and AC-9 no longer restate the quire-rs revision; IT-145 checks that the lockfile resolves the revision and version `Cargo.toml` pins, instead of asserting one hardcoded SHA across the manifest, lockfile, changelog and this specification. Every engine bump previously had to resync that SHA through prose, which guarded nothing `cargo --locked` does not. The engine moves to quire-rs 0.47.1 (`semantic_core` 0.3.0).
