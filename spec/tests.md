...
| IT-080 | `quire fix --scope <DIR> --module $M` with no positional uses `<DIR>/spec` as root; a `..`/symlink-escape on root or `--module` is rejected by path-safety before any load | Integration | P0 | FR-015-AC-5, FR-005 | 🚧 |
