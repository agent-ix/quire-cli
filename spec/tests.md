...
| FR-018 properties subcommand (per-criterion property classification) | AC-1..6 | IT-093 (census), IT-094 (JSON records + determinism), IT-095 (no criteria → empty, exit 0), IT-096 (non-extractable never fails), IT-097 (path-safety), TC-090 (thin boundary) | ✅ Complete |
