...
| IT-141 | A valid empty bounded corpus returns a complete successful envelope and exit 0; a missing document root or unreadable/invalid input returns exit 1 and empty stdout, so empty and unavailable cannot alias (`cli_assurance::it_141_*`) | Integration | P0 | FR-020-AC-4 | 🚧 |
...
| IT-144 | Rust `jsonschema`, Node/Ajv, and Python `jsonschema` accept the exact checked-in assurance golden without rewriting it; unavailable validators are named skips locally rather than fabricated passes (`tests/assurance_cross_language.rs`) | Integration | P0 | FR-020-AC-8 | 🚧 |
