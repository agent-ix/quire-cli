...
| FR-017 coverage subcommand (declarative rollup over two roots; agent-sized projection — #53; #51 close-out over quire-rs v0.42.0) | AC-1..19 | IT-107, IT-108 (CR-011 the engine capabilities reach a command line), IT-109 (finding lines lead with `row_id` — #51), IT-110 (`coverage` severity pack — off/error/reject-before-read), IT-111 (`--format tsv` records on stdout, `line` column populated), IT-112 (`--json` honours `--pretty`, compact default), IT-113 (`document:line` locus on finding lines → AC-16), IT-114 (`no_symbol_rows` renders in the census → AC-17), IT-115 (excluded-count census line + extraction diagnostics on stderr → AC-18), IT-116 (`shared_trace_ids` through `--json` → AC-19), IT-089 (human census on stderr), TC-740 (JSON payload byte-identical + no-model refusal), TC-810 (document root is `<scope>/spec`), IT-087 (code walk excludes it), TC-811 + IT-086 (missing root named + typed kind), IT-092 (`--strict`), TC-797 (zero matched rows ≠ full coverage), TC-090 (thin boundary) | ✅ Complete |
