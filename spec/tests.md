...
| FR-020 clauses subcommand (generic module clause sets) | AC-1..7 | IT-137 (evaluate + three-valued JSON + provenance/schema), IT-138 (exact-version diff + schema), IT-139 (deterministic TSV), IT-140 (fail-closed inputs and version selection), TC-141 (TSV structural escaping) | ✅ Complete |
...
| IT-137 | `clauses evaluate` selects one exact synthetic set, returns binding with supplied context and unresolved with missing context, carries the `clause_sets` capability, and conforms to the pinned engine's `clause-binding-v1` schema (`cli_clauses::it137_*`) | Integration | P0 | FR-020-AC-1, FR-020-AC-2, FR-020-AC-3, FR-020-AC-6 | ✅ |
| IT-138 | `clauses diff` compares exact versions and reports the added and changed synthetic clauses in a payload conforming to `clause-diff-v1` (`cli_clauses::it138_*`) | Integration | P0 | FR-020-AC-4, FR-020-AC-6 | ✅ |
| IT-139 | Clause evaluation TSV has a stable five-column header and one fully columned record per clause across repeated runs (`cli_clauses::it139_*`) | Integration | P1 | FR-020-AC-3, FR-020-AC-5 | ✅ |
| IT-140 | Malformed context and an unavailable exact version both exit 1 with an actionable diagnostic; no nearest version is guessed (`cli_clauses::it140_*`) | Integration | P0 | FR-020-AC-1, FR-020-AC-2, FR-020-AC-7 | ✅ |
