...
| IT-136 | `--module` is repeatable and the declared set is closed and ordered: two named modules both contribute their `traceability:` model, a module reachable only from `IX_FILAMENT_MODULES_PATH` contributes nothing, and a module named once emits no duplicate-module/archetype diagnostic despite an ambient same-named copy (`cli_coverage::it136_*`) | Integration | P0 | FR-017-AC-20 | ✅ |
| IT-137 | `quire coverage --help` states the `--module` resolution order — roots used in the order given, replacing ambient discovery rather than adding to it (`cli_coverage::it137_*`) | Integration | P1 | FR-017-AC-21 | ✅ |
