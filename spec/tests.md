...
| StR-004 Thin boundary | FR-002..004, FR-009, FR-011, FR-014, FR-020, NFR-005 | TC-090, TC-814 (src audits and review-checklist gate), IT-033..038, IT-136 | 🚧 (VC-1 repository-wide audit pending; VC-2/3 backed) |
...
| IT-136 | `quire assurance` over the pinned complete fixture emits the upstream `quire-assurance` v1 envelope with artifacts, obligations, symbols, relation kinds, resolved/dangling corpus edges, distinct verifies/implements relations, locators, and every observation availability state (`cli_assurance::it_136_*`) | Integration | P0 | FR-020-AC-1 | ✅ |
...
| IT-138 | Compact and `--pretty` invocations are each byte-identical on repeat; their parsed JSON values are equal, compact stdout before its newline equals `AssuranceExport::to_json_bytes()`, and pretty output is re-indented from those validated compact bytes (`cli_assurance::it_138_*`) | Integration | P0 | FR-020-AC-2 | ✅ |
| IT-139 | Wrong expected module name/version and missing, extra, or wrong schema premises each exit 1, name the rejected premise on stderr, and emit zero stdout bytes (`cli_assurance::it_139_*`) | Integration | P0 | FR-020-AC-3 | ✅ |
...
| IT-145 | The help snapshot, README command synopsis, changelog `assurance_export.v1` entry and capability token agree, and `Cargo.toml` pins quire-rs by `rev` with an exact `=` version (`cli_assurance_contract.rs`) | Integration | P0 | FR-020-AC-9 | ✅ |
| IT-146 | `--module` is repeatable and the declared set is closed and ordered: two named modules both contribute their `traceability:` model, a module reachable only from `IX_FILAMENT_MODULES_PATH` contributes nothing, and a module named once emits no duplicate-module/archetype diagnostic despite an ambient same-named copy (`cli_coverage::it146_*`) | Integration | P0 | FR-017-AC-20 | ✅ |
...
| IT-154 | `clauses evaluate` selects one exact synthetic set, returns binding with supplied context and unresolved with missing context, carries the `clause_sets` capability, and conforms to the pinned engine's `clause-binding-v1` schema (`cli_clauses::it137_*`) | Integration | P0 | FR-021-AC-1, FR-021-AC-2, FR-021-AC-3, FR-021-AC-6 | ✅ |
| IT-155 | `clauses diff` compares exact versions and reports the added and changed synthetic clauses in a payload conforming to `clause-diff-v1` (`cli_clauses::it138_*`) | Integration | P0 | FR-021-AC-4, FR-021-AC-6 | ✅ |
| IT-156 | Clause evaluation TSV has a stable five-column header and one fully columned record per clause across repeated runs (`cli_clauses::it139_*`) | Integration | P1 | FR-021-AC-3, FR-021-AC-5 | ✅ |
| IT-157 | Malformed context and an unavailable exact version both exit 1 with an actionable diagnostic; no nearest version is guessed (`cli_clauses::it140_*`) | Integration | P0 | FR-021-AC-1, FR-021-AC-2, FR-021-AC-7 | ✅ |
| IT-158 | `clauses evaluate` opens no network socket and spawns no process under `strace -fe network,process`; a missing `strace` fails the gate rather than passing it (`audit_no_network::clauses_does_not_open_inet_socket`) | Integration | P0 | FR-021-AC-7, NFR-004-AC-2 | ✅ |
...
| IT-165 | `verify_published` fetches the launcher package as published, by registry query, and resolves every platform package its published `optionalDependencies` names, through a real `npm view` subprocess call: a hermetic stub asserts the exact scope-registry override (`--@agent-ix:registry=…`) it is given and fails the call if it is wrong or absent — proving a dropped registry argument is caught, not silently ignored — and covers the launcher itself not resolving, one platform package pinned to a version that does not exist, `npm` printing a version that disagrees with what was requested, and an empty/absent `optionalDependencies` map failing outright rather than passing as a zero-iteration loop; a second, `#[ignore]`d case runs the same check against the real `npm` binary and a non-resolving registry host, proving the unreachable-registry path against genuine network failure rather than a simulated one | Integration | P0 | FR-023-AC-9 | ✅ |
| TC-815 | An `ix-trace-rs` Rust audit proves the typed target catalog has exactly four unique rows, the generated launcher dependencies are drawn only from those rows, and the Node host contains no second allowlist or behavior beyond ADR-0002; production-derived mutations must fail | Static | P0 | FR-022-AC-1, FR-022-AC-7, FR-023-AC-1, NFR-008-AC-1 | ✅ |
...
| TC-817 | Production-derived empty, extra, wrong-name, wrong-format, and cross-target binary artifact sets are each rejected before any existing output or launcher byte changes | Unit | P0 | FR-023-AC-2, FR-023-AC-3, FR-023-AC-7 | ✅ |
| TC-818 | Rust version-contract tests accept complete SemVer, validate the complete proposed state before replacement, update only the governed Cargo/npm fields, accept a launcher declaring a non-empty proper subset of the catalog, and kill malformed-version, malformed-manifest, empty/extra-dependency, and mismatch mutations without partial preflight writes | Unit | P0 | FR-024-AC-1, FR-024-AC-2, FR-024-AC-3, FR-024-AC-4 | ✅ |
| TC-819 | A production-derived Rust audit enumerates every `.yml`/`.yaml` release workflow and npm executable path, requires manual dispatch and Rust package/version/publish-verification commands (including `verify-published`), requires every publish-adjacent step to be gated on `inputs.publish`, and rejects executable MJS, Perl, embedded Node mutation, npm target/version assertions, automatic publication, or a second JavaScript entry | Static | P0 | FR-023-AC-9, FR-024-AC-5, FR-024-AC-6, NFR-008-AC-1 | ✅ |
...
| TC-822 | A build providing only a non-empty proper subset of the catalog (linux-x64 alone — the local publish path's actual shape, PLAT-885) succeeds, emitting exactly one npm package and one launcher `optionalDependency` for the platform present, and none for the three it did not build | Unit | P0 | FR-023-AC-2, FR-023-AC-4, FR-023-AC-6 | ✅ |
