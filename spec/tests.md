...
| IT-165 | `verify_published` resolves every launcher `optionalDependency` against a registry through a real `npm view` subprocess call (a hermetic stub standing in for the registry): every declared package resolving succeeds, one package pinned to a version that does not exist fails and names it, and an empty `optionalDependencies` map fails outright rather than passing as a zero-iteration loop | Integration | P0 | FR-023-AC-9 | ✅ |
