...
| FR-023-AC-9 | After publish, a separate command fetches the launcher package as actually published at the release version — from the supplied npm registry, never from the local working tree — and asserts both that the launcher itself resolves there and that every platform package its *published* `optionalDependencies` names also resolves at its declared version. It fails on any package/version that does not resolve, on a registry it cannot reach, and on an empty or absent `optionalDependencies` map (never a zero-iteration pass). It queries the registry through the package's own npm scope (`--@agent-ix:registry=…`), which npm gives precedence over a bare `--registry` flag — a bare `--registry` is silently inert for every package this tool checks and was the actual reason the first cut of this gate always passed. It is invoked as a required, registry-dependent step on the manually dispatched publish path (FR-024-AC-5), guarded the same as every other publish step. `verify_binary_version` does not cover any of this — it compares only the local binary's own reported version and asserts nothing about what a registry actually serves. | Test (IT-165), Static (TC-819) |
