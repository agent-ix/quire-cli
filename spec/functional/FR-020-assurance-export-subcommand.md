...
| FR-020-AC-4 | A valid corpus with zero artifacts, obligations, symbols, or relations still emits the complete successful envelope and exits zero, while unreadable/invalid inputs exit non-zero with empty stdout. | Test (IT-141) |
...
| FR-020-AC-8 | A checked-in golden JSON fixture is accepted by Rust, TypeScript, and Python schema validators without normalization, and pins every v1 field and state token. | Test (IT-144) |
