...
| FR-004-AC-19 | A **relative document path** given with `--scope <DIR>` resolves under that scope and validates against modules discovered from it, with no `--module` argument: a conformant document exits **0** with empty stdout and empty stderr | Test |
