...
| FR-024-AC-3 | The command validates the complete proposed Cargo and launcher state before its first replacement; a malformed launcher manifest, an empty optional-dependency set, or a dependency naming a platform outside the catalog leaves every governed file byte-identical. A launcher declaring a non-empty proper subset of the catalog is a valid input, not a mutation to reject — it is bumped like any other. | Mutation test (TC-818) |
